...
async function main() {
  console.log('[INFO] Iniciando diagnóstico do formulário de Editar Script...');
  ...
  try {
    ...
    // Clicar em "Editar" ao lado de sync-diario
    console.log('[INFO] Clicando em "Editar" no script sync-diario...');
    const editBtn = page.locator('button').filter({ hasText: /^Editar$/ }).first();
    await editBtn.click();
    await sleep(2000);
    
    await takeScreenshot(page, 'diagnose_edit_script_form');

    // Listar inputs e placeholders
    const inputs = await page.locator('input, textarea, select').all();
    console.log(`[INFO] Encontrados ${inputs.length} elementos de formulário na tela de edição.`);
    ...
    console.error('[ERRO DIAGNÓSTICO EDIT FORM]', err);
  } finally {
    await context.close();
  }
}
